...
link.f_fix.on.k <- list(
  mu = \(x) gamma_t(x, 0.1, 1),
  K = \(x) gamma_t(x, 0.15*10000, 10000),
  alpha = \(x) gamma_t(x, 1, 0.5),
  c_ = \(x) gamma_t(x, 0.1, 1),
  p = \(x) gamma_t(x, 0.1, 0.5)+1
)
...
inv.link.f_fix.on.k <- list(
  mu = \(x) inv_gamma_t(x, 0.1, 0.1),
  K = \(x) inv_gamma_t(x, 0.15*10000, 10000),
  alpha = \(x) inv_gamma_t(x, 1, 0.5),
  c_ = \(x) inv_gamma_t(x, 0.1, 1),
  p = \(x) inv_gamma_t(x, 0.1, 0.5)+1
)
...
link.f_fix.on.alpha <- list(
  mu = \(x) gamma_t(x, 0.1, 1),
  K = \(x) gamma_t(x, 1, 0.5),
  alpha = \(x) gamma_t(x, 2.5*10000, 10000),
  c_ = \(x) gamma_t(x, 0.1, 1),
  p = \(x) gamma_t(x, 0.1, 0.5)+1
)
...
inv.link.f_fix.on.alpha <- list(
  mu = \(x) inv_gamma_t(x, 0.1, 0.1),
  K = \(x) inv_gamma_t(x, 1, 0.5),
  alpha = \(x) inv_gamma_t(x, 2.5*10000, 10000),
  c_ = \(x) inv_gamma_t(x, 0.1, 1),
  p = \(x) inv_gamma_t(x, 0.1, 0.5)+1
)
...
link.f_fix.on.c <- list(
  ...
  K = \(x) gamma_t(x, 1, 0.5),
  alpha = \(x) gamma_t(x, 1, 0.5),
  c_ = \(x) gamma_t(x, 0.1*10000, 10000),
  p = \(x) gamma_t(x, 0.1, 0.5)+1
)
...
inv.link.f_fix.on.c <- list(
  ...
  K = \(x) inv_gamma_t(x, 1, 0.5),
  alpha = \(x) inv_gamma_t(x, 1, 0.5),
  c_ = \(x) inv_gamma_t(x, 0.1*10000, 10000),
  p = \(x) inv_gamma_t(x, 0.1, 0.5)+1
)
...
link.f_fix.on.p <- list(
  ...
  alpha = \(x) gamma_t(x, 1, 0.5),
  c_ = \(x) gamma_t(x, 0.1, 1),
  p = \(x) gamma_t(x, 0.5*10000, 10000)+1
)
...
inv.link.f_fix.on.p <- list(
  ...
  alpha = \(x) inv_gamma_t(x, 1, 0.5),
  c_ = \(x) inv_gamma_t(x, 0.1, 1),
  p = \(x) inv_gamma_t(x, 0.5*10000, 10000)+1
)
...
bind.post <- rbind(post.list_all$post.df, 
  post.list_fix.on.alpha$post.df[!post.list_fix.on.alpha$post.df$param=='alpha',],
        post.list_fix.on.c$post.df[!post.list_fix.on.c$post.df$param=='c',], 
        post.list_fix.on.mu$post.df[!post.list_fix.on.mu$post.df$param=='mu',],
        post.list_fix.on.k$post.df[!post.list_fix.on.k$post.df$param=='K',], 
        post.list_fix.on.p$post.df[!post.list_fix.on.p$post.df$param=='p',])
...
inv.link.f_mis.p <- list(
  ...
  alpha = \(x) inv_gamma_t(x, 1, 0.5),
  c_ = \(x) inv_gamma_t(x, 0.1, 1),
  p = \(x) inv_gamma_t(x, 4*10000, 10000)+1
)
...
inv.link.f_mis.c <- list(
  ...
  K = \(x) inv_gamma_t(x, 1, 0.5),
  alpha = \(x) inv_gamma_t(x, 1, 0.5),
  c_ = \(x) inv_gamma_t(x, 2*10000, 10000),
  p = \(x) inv_gamma_t(x, 0.1, 0.5)+1
)
...
inv.link.f_mis.k <- list(
  mu = \(x) inv_gamma_t(x, 0.1, 0.1),
  K = \(x) inv_gamma_t(x, 1.5*10000, 10000),
  alpha = \(x) inv_gamma_t(x, 1, 0.5),
  c_ = \(x) inv_gamma_t(x, 0.1, 1),
  p = \(x) inv_gamma_t(x, 0.1, 0.5)+1
)
...
link.f_mis.alpha <- list(
  mu = \(x) gamma_t(x, 0.1, 1),
  K = \(x) gamma_t(x, 1, 0.5),
  alpha = \(x) gamma_t(x, 8*10000, 10000),
  c_ = \(x) gamma_t(x, 0.1, 1),
  p = \(x) gamma_t(x, 0.1, 0.5)+1
)
...
inv.link.f_mis.alpha <- list(
  mu = \(x) inv_gamma_t(x, 0.1, 0.1),
  K = \(x) inv_gamma_t(x, 1, 0.5),
  alpha = \(x) inv_gamma_t(x, 8*10000, 10000),
  c_ = \(x) inv_gamma_t(x, 0.1, 1),
  p = \(x) inv_gamma_t(x, 0.1, 0.5)+1
)
...
```{r}
# bind marginal posterior data.frames
bind.post.mis <- rbind(post.list_all$post.df, 
        post.list_mis.p$post.df[!post.list_mis.p$post.df$param=='p',],
        post.list_mis.c$post.df[!post.list_mis.c$post.df$param=='c',], 
        post.list_mis.k$post.df[!post.list_mis.k$post.df$param=='k',], 
        post.list_mis.alpha$post.df[!post.list_mis.alpha$post.df$param=='alpha',],
