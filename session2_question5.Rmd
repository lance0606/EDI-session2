...
time.between.events.df <- lapply(multi.synth.cat.list.df, \(x) diff(x[x$gen==1,]$ts))
```
...
plot(density(time.between.events.df[[1]]),
     ...
     xlab='time between evetns',ylim=c(0,0.3))
for (i in 2:1000) {
...
```{r}
time.between.events.synth<-diff(synth.cat.df[synth.cat.df$gen==1,]$ts)
